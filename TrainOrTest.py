...
#        if (np.mod(i,10) == 0):
#          with open("epsilon.txt", "a") as myfile:
#            myfile.write(str(i) + " " + str(epsilon) + "\n")
...
#        epsilon = max(epsilon, 0.0)   
        ...
        for j in range(msteps):
            ...
            if (args['noise'] == 'OU'):
               ...
               a = actor.predict(np.reshape(s, (1, actor.s_dim)),noise=False) 
#            elif (args['noise']=='param'):
#               a = actor.predict(np.reshape(s, (1, actor.s_dim)),noise=True) 
            else:
            ...
            if replay_buffer.size() > int(args['minibatch_size']):
                ...
                #predicted_q_value, _ = critic.train(s_batch, a_batch, np.reshape(y_i, (int(args['minibatch_size']), actor.a_dim)))
